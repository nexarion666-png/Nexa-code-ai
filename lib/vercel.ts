...
export async function getVercelProject(projectId: string, token: string) {
  return vercelFetch(`/v9/projects/${encodeURIComponent(projectId)}`, token);
}

export async function listVercelProjects(token: string) {
  return vercelFetch('/v9/projects?limit=100', token);
}
...
export async function deployVercelProject(input: {
  ...
  return vercelFetch('/v13/deployments', input.token, {
    method: 'POST',
    body: JSON.stringify({
      name: input.name,
