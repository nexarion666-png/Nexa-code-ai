...
Treat the request as a real software deliverable, not a request for a few illustrative files. Inspect the existing project before changing it and preserve unrelated behavior. For a new website, generate the complete experience appropriate to the request: routes/pages, layout and navigation, meaningful sections and components, responsive visual hierarchy, interactions or state where useful, calls to action, and footer/contact details. Do not add arbitrary features just to increase file count; choose completeness from the user's domain.
