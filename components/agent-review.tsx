...
type Run = {
  ...
  verification?: {
    phase?: string;
    ok?: boolean;
    build?: { attempted?: boolean; passed?: boolean; command?: string; reason?: string };
    latest?: {
      phase?: string;
      ok?: boolean;
      build?: { attempted?: boolean; passed?: boolean; command?: string; reason?: string };
...
export function AgentReview({ runId, onApproved }: { runId?: string; onApproved?: () => void }) {
  ...
  async function approve() {
    ...
    try {
      ...
      if (!response.ok) {
       ...
       setNotice(data.summary ?? (data.status === 'completed' ? 'Approved, verified, and implemented.' : 'Applied, but verification did not pass.'));
      onApproved?.();
    } catch (error) {
      setNotice(
        error instanceof Error
  ...
  const verification = run?.verification?.latest ?? run?.verification;
    ...
    <div className="diff-list">
      ...
      {run?.status && (
        <div className="muted" style={{ marginBottom: 12 }}>
          ...
          {verification?.build?.attempted
            ? ` · production build ${verification.build.passed ? 'passed' : 'failed'}`
